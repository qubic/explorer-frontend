...
const Legend = React.forwardRef<HTMLOListElement, LegendProps>((props, ref) => {
  ...
  return (
    <ol ref={ref} className={clsxTwMerge('relative overflow-hidden', className)} {...other}>
      <div
        ref={scrollableRef}
        tabIndex={0}
        className={clsxTwMerge(
...
const ChartTooltip = ({ active, payload, label, valueFormatter }: ChartTooltipProps) => {
  ...
  if (active && payload && payload.length) {
    return (
      ...
      >
        <div className={clsxTwMerge('border-b border-inherit px-4 py-2')}>
          <p
            className={clsxTwMerge(
              // base
              'font-medium',
...
const AreaChart = React.forwardRef<HTMLDivElement, AreaChartProps>((props, ref) => {
  ...
  return (
    <div
      ref={ref}
      className={clsxTwMerge('h-80 w-full', className)}
    ...
    >
      <ResponsiveContainer>
        ...
        >
          {showGridLines ? (
            <CartesianGrid
              className={clsxTwMerge('stroke-gray-200 stroke-1 dark:stroke-gray-800')}
              horizontal={true}
              vertical={false}
            />
          ) : null}
          ...
          {onValueChange
            ? categories.map((category) => (
                <Line
                  className={clsxTwMerge('cursor-pointer')}
                  strokeOpacity={0}
                  key={category}
                  name={category}
                  type="linear"
